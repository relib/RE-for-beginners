\mysection{Breaking simple executable code encryptor}

We've got an executable file which is encrypted by relatively simple encryption.
\href{\RepoURL/examples/simple_exec_crypto/files/cipher.bin}{Here is it} (only executable section is left here).
...
The problem is to recover \ac{IV}.
Brute-force is also not an option, because \ac{IV} is too long (16 bytes).
Let's see, if it's possible to recover \ac{IV} for arbitrary encrypted executable file?
...
If compiler needs to encode value bigger than 127, it has to use 32-bit displacement instead of 8-bit one, but large values are very rare (\myref{MostPopularNumbers}),
...
Now about 00 and 0xFF bytes combined: jumps (including conditional) and calls can pass execution flow forward or backwards, but very often,
within the limits of the current executable module.
If forward, displacement is not very big and also padded with zeros.
If backwards, displacement is represented as negative value, so padded with 0xFF bytes.
...
So far so good. Now we have to try various 16-byte keys, decrypt executable section and measure how often 0, 0xFF and 0x8B bytes are occurred.
...
Now I'm trying all possible bytes (0..255) for each byte in key and just pick the byte producing maximal amount of 0/0xFF/0x8B bytes in a decrypted slice:
...
(Source code can be downloaded \href{\RepoURL/examples/simple_exec_crypto/files/decrypt.py}{here}.)

I run it and here is a key for which 0/0xFF/0x8B bytes presence in decrypted buffer is maximal:
...
(Source code can be downloaded \href{\RepoURL/examples/simple_exec_crypto/files/decrypt2.py}{here}.)
...
The whole decryped file can be downloaded \href{\RepoURL/examples/simple_exec_crypto/files/decrypted.bin}{here}.
...
(Hence, this is average length of PUSH instructions chain.)
