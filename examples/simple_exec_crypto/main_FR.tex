...
% to be sync: \mysection{Breaking simple executable code encryptor}

J'ai un fichier exécutable qui est chiffré par un chiffrement relativement simple.
% to be sync: We've got an executable file which is encrypted by relatively simple encryption.
...
% to be sync: The problem is to recover \ac{IV}.
...
déplacement 32-bit au lieu d'un de 8-bit, mais les grandes valeurs sont très rares (\myref{MostPopularNumbers}),
...
% to be sync: \item Many modern compilers aligns beginnings of functions at 16-byte border.
Donc l'espace libre avant est rempli avec de NOPs (0x90) ou d'autres instructions
avec des opcodes connus: \myref{sec:npad}.
% to be added: Or INT3 instructions (0xCC).
...
% to be sync: (Hence, this is average length of PUSH instructions chain.)
