...
A demo build can lack of some important functions, as with this example:

\lstinputlisting[caption=\CCpp code,style=customc]{patterns/00_empty/demo_EN.c}

The \TT{save\_file()} function could be called when the user clicks \TT{File->Save} on the menu.
The demo version might be delivered with this menu item disabled, but even if a software cracker would enable it,
