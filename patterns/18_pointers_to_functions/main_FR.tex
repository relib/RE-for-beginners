...
\myindex{Callbacks}
Ils sont souvent utilisés pour appeler des fonctions callback
(de rappel).

Des exemples bien connus sont:

\begin{itemize}
\item \qsort, \TT{atexit()} de la bibliothèque C standard; 

\item signaux des OS *NIX;

\item démarrage de thread: \TT{CreateThread()} (win32), \TT{pthread\_create()} (POSIX);

\item beaucoup de fonctions win32, comme \TT{EnumChildWindows()}.

\item dans de nombreux endroits du noyau Linux, par exemple les fonctions des drivers
du système de fichier sont appelées via callbacks.
